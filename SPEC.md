...
| 模块 | 职责 | 状态 |
|------|------|------|
| 用户认证 | 注册、登录、JWT 鉴权、学分系统 | ✅ 已有 |
| 需求发布 | 发布/编辑/取消需求，AI 智能分析推荐 | ✅ 已有 + AI 增强 |
| 订单管理 | 接单、状态流转、验收 | ✅ 已有 |
| 支付系统 | 信用积分支付 | ✅ 已有 |
| 评价系统 | 双向评价、信用计算 | ✅ 已有 |
| 消息系统 | 站内消息通知 | ✅ 已有 |
| **AI 需求分析** | **LLM 自动分析需求描述，推荐分类/定价/标签** | 🆕 新增 |
| **校园 AI 助手** | **对话式 AI 助手，帮助用户了解平台、搜索需求** | 🆕 新增 |
| 凭据管理 | AI API Key 安全存储（AES-256-GCM 加密） | 🆕 新增 |
...
| 层级 | 技术 | 选型理由 |
|------|------|----------|
| 后端 | Node.js 22 + Express 5 | 原 CampusHub 项目技术栈，团队熟悉；Express 5 原生支持 async/await |
...
- **平台**：Railway / Render（免费额度）
- **架构**：前端 Nginx 静态服务 + 后端 Node.js API + MySQL 数据库
- **CI/CD**：GitHub Actions 自动构建 Docker 镜像，推送到 GitHub Container Registry
